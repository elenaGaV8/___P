...
class Player(GameSprite):
    def update_l(self):
        ...
        if keys_pressed[K_s] and self.rect.y < 200:
            self.rect.y += self.speed
    def update_r(self):
        keys_pressed = key.get_pressed()
        if keys_pressed[K_UP] and self.rect.y > 25:
            self.rect.y -= self.speed
        if keys_pressed[K_DOWN] and self.rect.y < 200:
            self.rect.y += self.speed
player_1 = Player('левая платформа.png', 50, 100, 4, 100, 300)
player_2 = Player('правая платформа.png', 550,100, 4, 100, 300)
ball = GameSprite('ball.png', 100, 100, 4, 50, 50)
speed_x = 3
speed_y =3
game = True
FPS = 60
